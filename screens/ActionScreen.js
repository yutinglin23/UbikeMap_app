...
const ActionScreen = ({ onClose, site }) => {
   ...
   return (
      ...
      <VStack h="50%" w="100%" bg="#F6E3FD" borderRadius={20} >
         ...
         <Text fontWeight={'bold'} fontSize="20" my={6} textAlign="center">{sna}{" "}站</Text>
         <Box px={6} >
            <Text fontSize="15"><Text fontWeight={'bold'} fontSize="15">地址：</Text>{sarea}{" "}{ar}</Text>
            <Text mt={2} fontSize="15"><Text fontWeight={'bold'} fontSize="15">經度/緯度：</Text>{Number(lng).toFixed(2)}/{Number(lat).toFixed(2)}</Text>
            <Text mt={2} fontSize="15"><Text fontWeight={'bold'} fontSize="15">更新時間：</Text>{getTime(mday)}</Text>
            <Center>
               <VictoryPie
                  width={250}
                  height={250}
